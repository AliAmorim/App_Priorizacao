...
export async function listarUserStories(squad) {
  const { data, error } = await getDb()
    .from('items')
    .select('numero, nome, po, pri, status, area, prazo, ordem, tipo, impedido')
    .eq('squad', squad)
    .order('ordem');
...
// Substitui a lista de squads que um usuário pode acessar.
export async function atualizarSquads(username, squads) {
  const key = String(username || '').trim().toLowerCase();
  const list = Array.isArray(squads) ? squads.filter(Boolean) : [];
  const { error } = await getDb()
    .from('users')
    .update({ squads: list })
    .eq('username', key);
  if (error) throw new Error(error.message);
  return { ok: true, username: key, squads: list };
}

// Remove um usuário.
export async function removerUsuario(username) {
  const key = String(username || '').trim().toLowerCase();
  const { error } = await getDb()
    .from('users')
    .delete()
